 ...
 */
void QuantumSearch::quantum_oracle(NoisyQuantumComputation &circuit)
{
...
{
    auto noise_operator = NoisyQuantumComputation(this->size());

    this->quantum_oracle(noise_operator);
    this->quantum_diffusion(noise_operator);

    auto circuit = noise_operator.build_noisy_qc();
    return circuit;
}
