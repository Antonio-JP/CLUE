...
for bench in $MQT_BENCH_NAMES; do
    for n in $(seq $REPS); do
        python3 q_benchmark.py -to 500 -n $bench -m 3 -M 7 -r 1;
    done
done
...
for bench in $MQT_BENCH_NAMES; do
    for n in $(seq $REPS); do
        python3 q_benchmark.py -to 500 -t full_ddsim -obs 0 -obs H -n $bench -m 3 -M 7 -r 1;
    done
done
...
for bench in $MQT_BENCH_NAMES; do
    for n in $(seq $REPS); do
        python3 q_benchmark.py -to 500 -t full_quokka# -obs 0 -obs H -n $bench -m 3 -M 7 -r 1;
    done
done